...
Medico ArchivoMedico::getData(const int & idx) {
    ...
    archivoPorNombre.open(nameFileMedico);
    if (!archivoPorNombre.is_open()) {
        throw ios_base::failure("Error al cargar el archivo en ArchivoMedico::getData()");
    }

    archivoPorNombre.seekg(idx);
    getline(archivoPorNombre, myString, '#');
    if (myString.empty()) {
...
/*void ArchivoMedico::delData(const int& idx) {
    archivo.open(nameFileMedico, ios_base::out);
    if (!archivo.is_open()) {
...
Lista<Medico> ArchivoMedico::getMisMedicos() {
    return misMedicos;
}
